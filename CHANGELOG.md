...
### Fixed
- **갱신 시 펼쳐둔 폴더가 모두 접히던 문제** — 루트 갱신 버튼은 트리를 remount해 새로고침하는데,
  폴더 펼침 상태가 각 트리 노드의 로컬 state에만 있어 remount와 함께 초기화됐음. 펼침 상태를
  워크스페이스별로 스토어에 보관·영속화(`expandedDirs`)하도록 바꿔, 갱신 후에도 열려 있던 폴더가
  그대로 유지된 채 내용만 다시 읽어옴. 로컬 폴더·GitHub 저장소 모두 동일하게 동작하며,
  앱을 다시 실행해도 펼침 상태가 복원됨
...
- **본문 전체 가로 폭 사용** — 마크다운 뷰(`.markdown-body`)의 `max-width: 920px` 중앙 정렬
  ...
  표·다이어그램이 전체 폭으로 펼쳐짐(VS Code 프리뷰와 동일한 동작)

...
- **Mermaid 다이어그램** — ```mermaid 코드펜스 렌더링(VS Code 스타일 색/폰트, 확대/축소·이동 컨트롤)
- **MD/TX 보기 모드** — 마크다운 렌더링 ↔ 원문(일반 텍스트) 전환 토글
- **서식 있는 복사** — 렌더된 모습 그대로 Word/PPT에 붙여넣기, 본문 우클릭 복사 메뉴
- **본문 확대/축소(줌 %)** — Ctrl +/−/0 · Ctrl+휠, 글씨·이미지·표 일관 스케일
...
- **v0.3.2** — Mermaid **확대(+) 미동작·드래그 시 배율 리셋** 수정, 다이어그램 보기 모드
  (**1:1 · 화면 맞춤 · 배율%**) 추가, 이동 기록 바 버튼 배치 정리
- **v0.3.0** — **Mermaid 다이어그램**(VS Code 스타일, 확대/축소·이동), **MD/TX 보기 모드**,
  **서식 있는 복사**(Word/PPT)·우클릭 메뉴, **본문 줌(%)**, 조직(Organization) 저장소 목록 보강
